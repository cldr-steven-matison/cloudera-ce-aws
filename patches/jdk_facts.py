...
VERSION_REGEX = re.compile(
    ...
    + "\\.?(?P<minor>\\d*)"
    + "\\.?(?P<patch>\\d*)"
    + "(?:\\.(?P<security>\\d+))?"  # PATCH (#292): four-component versions, e.g. 17.0.20.1
    + "[+-_]?(?P<release>[\\w\\d]*)"
    + "[+-_]?(?P<build>[\\w\\d]*)"
    + "[^)]*"  # PATCH (#292): tolerate any trailing suffix inside the parentheses
    + ")"
    + "\\)",
)
...
def main():
    ...
    if rc != 0:
        ...
        if version is None:
            # PATCH (#292): degrade gracefully instead of crashing on an unparsed version line
            module.warn(f"Unable to parse JDK version from: {output[1]}")
            result["ansible_facts"]["jdk"].update(
                provider="OpenJDK", version=None, major=None, minor=None, patch=None,
                release=None, build=None, update=(update[1] if update is not None else ""),
            )
        else:
            result["ansible_facts"]["jdk"].update(
                provider=(
